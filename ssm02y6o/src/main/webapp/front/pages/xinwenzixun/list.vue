		...
		<view :style='{"width":"100%","padding":"0","position":"relative","flexWrap":"wrap","display":"flex","height":"100%"}'>
			<view class="cu-bar bg-white search" style="width:100%" :style="[{top:CustomBar + 'px'}]">
				<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
					...
					<view v-if="queryIndex==0" class="search-form round">
						...
						<input v-model="searchForm.zhuti" type="text" placeholder="主题" ></input>
					</view>
				<view class="action">
			...
			<view class="uni-product-list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","flex":"1","display":"flex","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"border":"6rpx solid #00B578","padding":"48rpx 0 0 ","margin":"0 0 20rpx","backgroundColor":"#fff","overflow":"hidden","borderRadius":"20rpx","width":"48%","position":"relative","height":"390rpx"}' v-for="(product,index) in list" :key="index">
					<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","overflow":"hidden","color":"#333","top":"0","textAlign":"center","background":"#fff","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","position":"absolute","height":"48rpx"}'>主题:{{product.zhuti}}</view>
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('xinwenzixun','修改')) || (!userid && isAuthFront('xinwenzixun','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('xinwenzixun','删除')) || (!userid && isAuthFront('xinwenzixun','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`xinwenzixun`, params);
                } else {
                    res = await this.$api.list(`xinwenzixun`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xinwenzixun', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.zhuti){
					searchForm['zhuti'] = '%' + this.searchForm.zhuti + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`xinwenzixun`, searchForm);
                } else {
                    res = await this.$api.list(`xinwenzixun`, searchForm);
                }
				// 如果是第一页数据置空
